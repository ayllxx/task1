  ...
  if (active && payload && payload.length) {
    ...
    return (
      <View style={styles.tooltipContainer}>
        ...
        <Text style={styles.tooltipValue}>{`${uv}`}</Text>
      </View>
    );
  }
  return null;
...
const InteractiveChart = () => {
  ...
  return (
    ...
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', height:229, width:748, left: -10 }}>
      <View style={styles.buttonContainer}>
        <TouchableOpacity
          style={[styles.button, timeframe === '12months' && styles.activeButton]}
          onPress={() => setTimeframe('12months')}
        >
          <Text style={styles.buttonText}>12 Months</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.button, timeframe === '6months' && styles.activeButton]}
          onPress={() => setTimeframe('6months')}
        >
          <Text style={styles.buttonText}>6 Months</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.button, timeframe === '30days' && styles.activeButton]}
          onPress={() => setTimeframe('30days')}
        >
          <Text style={styles.buttonText}>30 Days</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.button, timeframe === '7days' && styles.activeButton]}
          onPress={() => setTimeframe('7days')}
        >
          <Text style={styles.buttonText}>7 Days</Text>
        </TouchableOpacity>
      </View>
      <ResponsiveContainer width={880} height={229}>
        <AreaChart data={data[timeframe]} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          <CartesianGrid strokeDasharray="" vertical={false} stroke="rgba(0, 0, 0, 0.1)" />
          ...
          <XAxis dataKey="shortName" axisLine={false} tickLine={false} fontFamily='PlusJakartaSans-Medium' fontSize={13} />
